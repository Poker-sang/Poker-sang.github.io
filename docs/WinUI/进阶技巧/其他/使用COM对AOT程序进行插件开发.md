---
title: 使用COM对AOT程序进行跨平台、跨语言的插件开发
date: 2025-01-14
category:
  - 进阶技巧
  - 其他
...
COM的缺点也很明显，是C++语言的接口，那它就只支持方法了，对于有各种奇技淫巧的C#接口来说略显原始。
...
OOP方式使用MsRpc框架，是跨进程通信，所以比较灵活，同时跨进程通信可能导致内存泄漏，效率也不是很高。而且这个方法需要系统相关的COM API支持，所以就不能实现跨平台了。[Dev Home](https://github.com/microsoft/devhome)就是使用这种方式实现的，感兴趣的可以看看它的源代码。
